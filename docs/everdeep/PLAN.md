...
| V23 | — | Audit round 2 (retracted): "sea route line invisible over water". | ✖ NOT A FINDING: the shot had NO route — the clicks never made a plan (see V24), and no sea route CAN plan today (V25). Judge sea-leg contrast once V25 lands; the boat dash (rgba(90,190,235)) may still want a dark casing then. |
...
| V25 | MED | Boat travel never plans on the shipped Earth example: every crossing that needs water says "No route on the chosen methods". | 🔴 OPEN (found chasing V24, proven pre-existing by stash bisect at b222). In-app probe matrix over the central Med, party-start flow, all four fail identically: Sicily→Peloponnese, Catania→Peloponnese, Sicily→Calabria (a 100-mi hop over the Messina strait), Calabria→Peloponnese. Endpoints verified land (grass/forest hex cards); pure-land plans work fine (Paris→Munich ≈120mi walks and rides). Suspect the embark layer: `travelPorts` needs a 10k+ settlement whose WORLD-tier hex sits on a river or beside a water/deep hex (mapView `buildTravelLayers`) — either the fixture's coastal cities fail that adjacency test at 60-mi grain or the port map is empty altogether. The 25-travel-sea audit shot currently documents this honestly: banner engaged, party on Sicily, "No route". Probe spec preserved at the session scratchpad as `probe-sea.spec.ts.keep`. |
