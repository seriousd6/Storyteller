...
- **(2026-07-16) Winds & currents on the map — how should they LOOK (#31a/#31b "mapped" half).**
  The fields exist and are tested (`windField.ts` b129, `currentField.ts` b131), but nothing draws them yet, because *how* is an aesthetic call on the live map. Options: (a) faint **streamlines** that flow along the wind/current — elegant, reads as a weather chart; (b) a sparse grid of **arrows** — legible, busier; (c) **animated** drift (particles running downstream) — lovely, a bigger render cost and it fights the new terrain buffer (b128). And: on by **default** or behind a toggle (like ⛵/👑)? Winds over land + sea, or currents over sea only? My lean: static streamlines, currents-on-sea + winds everywhere, behind a new "🌬 winds" toggle off by default. Say the word and I'll build it.
- **(2026-07-16) Boat travel — how hard should wind/current bite (#31c/#31d wiring).**
  The sailing model is built and tested (`sailing.ts` b132) but NOT wired into the travel A* yet, because the *strength* is a gameplay-feel call. Two knobs: (1) how much a fair/foul current changes a crossing (now `CURRENT_STRENGTH=0.4` of hull speed — enough that a becalmed raft can't beat a current, per your rule); (2) whether the travel tool sails **unpowered by default** (so wind/current genuinely route the trip) with a "powered" toggle, or powered by default with unpowered as the interesting mode. Also: should a fantasy-world's "magic hull" for 500k+ cities (the portal-tier boats) count as powered? My lean: unpowered default, powered toggle, magic hulls = powered. Confirm and I'll wire it.
