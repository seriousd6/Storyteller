...
|---|---|---|---|
| V26 | HIGH | **Roads die within sight of the road they should join** — 917 endpoints (32%) end 0.5–6 mi from another road; 898 pairs run <5 mi apart and never touch. Shots: three collinear fragments each dying in forest within sight of the next (t-dangle-ganges-cluster, ~86°E 27°N); two parallel roads ~0.45 mi apart for miles below Bright Skopje (t-nearmiss-skopje). | The geometric-merge pass (settlements.ts, "#10b geometric merge") cuts a run the moment it shadows a trunk (±2-cell heading claim) and "re-emerges where it diverges" — but never lands the cut end ON the trunk it yielded to, so the reader sees a road stop a mile short of the road that replaced it. Fix shape: every merge-cut end gets a short junction segment to the nearest point of the CLAIMING trunk (same idiom as the doorstep links, which already do this for plan ends); after that, re-run the probe — the 898 near-miss pairs should collapse to genuine geography. |
| V27 | HIGH | **Roads to nowhere**: 292 endpoints in open wilderness (62 of them >30 mi from any settlement or road). Shots: Deep Milan's ONLY road peters out mid-forest (r3-04); a lone road starting in the wild near fantasy-Dakar (t-dangle-dakar). | Leftovers of two knives: hugLand's water splits (the prune pass only drops a stub when its END faces water — a stub whose CUT is inland survives) and merge cuts whose trunk was itself later cut. Fix: after all cutting, one connectivity sweep — a free end that reaches neither settlement, junction, nor water within ~3 mi either extends to the nearest network point (≤ LINK_MAX) or drops, under the existing never-strand-a-town guard. |
| V28 | HIGH | **Roads spin around cities**: 71 routes return to within 1 mi of themselves after 8+ mi of arc. Shot: the Belem Bastion road sweeps a ~106-mi lasso through Near Beechmeadow and dies next to its own outbound line (t-loop-belem). 20 of the 71 pass through a settlement (Malagashire, Chibafell, Old Karachimarch…). | Planner-level: spur links chain town→nearestRoadPoint on 60-mi CELL CENTres, so a chain of spurs can walk a C around a city and close the horseshoe without ever joining. Fix: when a route's end passes within ~1 mi of ITS OWN line (or its chain's), close the loop with a junction instead of running on; and reject a spur link whose arc length is ≫ its crow-flies gain (the 106-mi lasso nets ~0.5 mi). |
| V29 | MED | **Roads stop short of their city**: 72 endpoints hang 0.5–3 mi from the settlement they serve. Shots: a road ends mid-field between Amber Foshan and Fort Guangzhou's walls (t-stopshort-guangzhou); both giant walled-metro footprints in that frame have ZERO roads touching the art. | The plan's node point and the drawn city art disagree (water-magnetism moved the node; the art centres on the anchor). Fix: extend the final segment to the settlement ANCHOR (or to the art's wall ring at city sizes) at emit time — the doorstep-link machinery already knows the anchor; it just stops at REACH_FT=3 mi, which is exactly the gap the shots show. Lower REACH to ~0.4 mi and aim at the anchor, not the cell. |
| V30 | HIGH | **Rivers run far into the open sea**: 149/481 rivers continue >4 mi past the coast through water — the Congo crosses the coast and keeps going ~44 mi to the frame corner (t-overshoot-congo), the Paraná sails ~44 mi across the open Plata (t-overshoot-plata), two Hudson Bay rivers thread ~90 mi of open bay (t-overshoot-hudson), one Baikal-region course logs 189 mi of water run. | Three feeders: authored courses are deliberately "closed to the real estuary" (bake-earth-rivers), `extendTransplantTails` appends tails that END at a water HEX CENTRE (up to ~45 mi offshore at 60-mi grain), and generated courses emit through the mouth hex. Fix at one chokepoint: clip every river polyline where it first crosses the coast MASK (the same `earthCoastLand`/biome read the probe used), keeping ~1–2 mi of estuary so mouths still visibly meet the sea; lakes exempt (a course may cross a lake it feeds). Re-bake; the probe count should drop to near the deliberate-estuary handful. |
| V31 | HIGH | **Boxy biomes, two quantizations**: (a) at city zoom the land cover is a checkerboard of flat axis-aligned squares (~95 px = the 12-mi raster cell; r3-04 Po valley, r3-06 Ganges); (b) at 10-mi zooms biome patches and shallow-water tiles are naked REGION HEXES — yellow sand hexes scattered on green, water hexes each wearing a full beach rim (r3-24 Danube delta, r3-26 Thames "hex-lake soup", t-loop-belem coast). | (a) the earth land-cover read is nearest-cell: add a seeded per-sample jitter of ~half a cell (same idiom as BIOME_WARP, which already exists for exactly this) or bilinear-vote the 2×2 neighbourhood, so cell edges dissolve at every zoom; (b) V13's edge dither only fires on world-tier hexes ≥14 px — extend the same treatment to region-tier land classes, and derive the beach band from coast DISTANCE (coastDistAt) instead of per-hex classification so sand follows the shoreline, not the tiling. |
| V32 | MED | **Rivers draw as blue staircases at city zoom** — the Rhine and Ganges ribbons are square-stepped tiles bank to bank (r3-03, r3-06). | Draw-time: the ribbon polygon follows the hex-quantized course samples with no bank smoothing at high ppf. Chaikin the ribbon OUTLINE (not the course — bridges/fords key on it) once its real width exceeds ~8 px, the same draw-only idiom V7 used on border loops. |
...
| V35 | LOW | Band-2 city rivers stop at the inner mouth, so estuary frames read as bare sea — no Thames anywhere in the estuary frame (r3-26), the Toneror label floats on an invisible line (r3-08). | Likely honest data (the authored band-2 course ends at the mouth point) colliding with V30's opposite sin. When V30's coast clip lands, decide the estuary treatment once for both: a course should END where the coast mask says sea begins — no shorter, no longer. |
| V36 | LOW | Metro pins float beside their trunk road with no visible spur (Verdant Nottingham r3-12, High Baltimorecliff r3-15) — the V29 gap at survey zoom, listed so the fix is measured against BOTH zooms. | Same fix as V29 (anchor-aimed doorstep links); kept separate so the re-shot checks survey zoom too. |
